...
public static class Scaffolder
{
    ...
    private static string GetProviderName(DbConnectionStringBuilder connectionStringBuilder)
    {
        ...
        return new DbContext(builder.Options).Database.ProviderName ?? throw new InvalidOperationException($"Provider name for {connectionStringBuilderName} could not be determined.");
    }
    ...
    private static void AddAutogeneratedComment(Settings settings, SavedModelFiles savedModelFiles)
    {
        Parallel.ForEach(savedModelFiles.AdditionalFiles.Append(savedModelFiles.ContextFile).Distinct(), filePath =>
        {
            ...
            lines.AppendLine("//");
            lines.AppendLine($"// Generated from {settings.GetDisplayableConnectionString(settings.ConnectionStringBuilder)}");
            lines.AppendLine("// ");
            lines.AppendLine("// </auto-generated>");
            lines.AppendLine();
