...
public static class Scaffolder
{
    ...
    private static SavedModelFiles ScaffoldContextAndEntities(Settings settings, string providerName)
    {
    ...
    private static void AddAutogeneratedComment(Settings settings, SavedModelFiles savedModelFiles, string providerName)
    {
        Parallel.ForEach(savedModelFiles.AdditionalFiles.Append(savedModelFiles.ContextFile).Distinct(), filePath =>
        {
            ...
            lines.AppendLine($"// ┌───────────────────┬─{dashesPadding}─┐");
            lines.AppendLine($"// │ Provider          │ {providerName}{new string(' ', 1 + maxLength - providerName.Length)}│");
            lines.AppendLine($"// ├───────────────────┼─{dashesPadding}─┤");
            lines.AppendLine($"// │ Connection String │ {connectionString}{new string(' ', 1 + maxLength - connectionString.Length)}│");
            lines.AppendLine($"// └───────────────────┴─{dashesPadding}─┘");
            lines.AppendLine("// ");
            lines.AppendLine("// </auto-generated>");
            lines.AppendLine();
